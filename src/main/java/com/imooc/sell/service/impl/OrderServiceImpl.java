...
public class OrderServiceImpl implements OrderService {
    ...
    public OrderDTO create(OrderDTO orderDTO) {
        ...
        for (OrderDetail orderDetail : orderDTO.getOrderDetailList()) {
            ...
            if (productInfo == null) {
                log.error("商品不存在" + orderDetail.getProductId());
    ...
    public OrderDTO cancel(OrderDTO orderDTO) {
        ...
        if (updateResult == null) {
            log.error("【取消订单】取消失败，orderId={}", orderDTO.getOrderId());
            throw new SellExcetion(ResultEnum.ORDER_UPDATE_FAIL);
        }

        // 返回库存
        if (CollectionUtils.isEmpty(orderDTO.getOrderDetailList())) {
            log.error("【取消订单】取消失败,订单为空，orderId={}", orderDTO.getOrderId());
    ...
    public OrderDTO finish(OrderDTO orderDTO) {
        ...
        orderMaster.setPayStatus(OrderStatusEnum.FINISHED.getCode());
        OrderMaster updateResult = orderMasterRepository.save(orderMaster);
        if (updateResult == null) {
